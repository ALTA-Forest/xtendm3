/**
 * README
 * This extension is being used to copy a contract template.
 * 
 * Name: EXT002MI.CpyContrTemp
 * Description:Copy a contract template with all the sections .
 * AFMNI-7/Alias Replacement
 * https://leanswift.atlassian.net/browse/AFMI-7

 * Date                      Changed By                          Description
 * 2023-04-10                Jessica Bjorklund                   Version 1 - intial approval
 * 2024-10-18                Backiyalakshmi Ramasubramanian      Version 2 - Adding data to EXTCSR
 * 2024-10-31                Jessica Bjorklund                   Version 3 - Change to get the last number from a number series instead
 */
...
public class CpyContrTemp extends ExtendM3Transaction {
  ...
  public void main() {
    ...
    for (DBContainer recLineEXTCDS: resultEXTCDS) {
      ...
      for (DBContainer recLineEXTCSR: resultEXTCSR) {
        ...
        Optional < DBContainer > EXTCSR = findEXTCSR(inCONO, inDIVI, Integer.parseInt(sectionIDString), Integer.parseInt(rateSeqString))
        if (!EXTCSR.isPresent()) {
           getNextNumber("", "LD", "1") 
           inSRID = nextNumber as Integer
           addEXTCSRRecord(inCONO, inDIVI, inSRID, Integer.parseInt(sectionIDRateString), Integer.parseInt(rateSeqString), Double.parseDouble(minLengthString), Double.parseDouble(maxLengthString), Double.parseDouble(minDiamString), Double.parseDouble(maxDiamString), Double.parseDouble(amountString), noteString)
        } else {
  ...
  private List < DBContainer > listEXTCTI(int CONO, String DIVI, String RVID) {
    ...
    query.readAll(EXTCTI, 3, pageSize, {
      DBContainer recordEXTCTI ->
  ...
  void addEXTCSRRecord(int CONO, String DIVI, int SRID, int DSID, int CRSQ, double CRML, double CRXL, double CRMD, double CRXD, double CRRA, String CRNO) {
    DBAction action = database.table("EXTCSR").index("00").selection("EXDSID", "EXCRSQ", "EXCRML", "EXCRXL", "EXCRMD", "EXCRXD", "EXCRRA", "EXCRNO").build()
  ...
  private List < DBContainer > listEXTCDS(int CONO, String DIVI, String RVID) {
    ...
    int pageSize = mi.getMaxRecords() <= 0 || mi.getMaxRecords() >= 10000 ? 10000 : mi.getMaxRecords()
    query.readAll(EXTCDS, 3, pageSize, {
      DBContainer recordEXTCDS ->
  ...
  private List < DBContainer > listEXTCSS(int CONO, String DIVI, int DSID) {
    ...
    int pageSize = mi.getMaxRecords() <= 0 || mi.getMaxRecords() >= 10000 ? 10000 : mi.getMaxRecords()
    query.readAll(EXTCSS, 3, pageSize, {
      DBContainer recordEXTCSS ->
  ...
  private List < DBContainer > listEXTCSG(int CONO, String DIVI, int DSID) {
    ...
    int pageSize = mi.getMaxRecords() <= 0 || mi.getMaxRecords() >= 10000 ? 10000 : mi.getMaxRecords()
    query.readAll(EXTCSG, 3, pageSize, {
      DBContainer recordEXTCSG ->
  ...
  private List < DBContainer > listEXTCSE(int CONO, String DIVI, int DSID) {
    ...
    int pageSize = mi.getMaxRecords() <= 0 || mi.getMaxRecords() >= 10000 ? 10000 : mi.getMaxRecords()
    query.readAll(EXTCSE, 3, pageSize, {
      DBContainer recordEXTCSE ->
  ...
  private List < DBContainer > listEXTCSR(int CONO, String DIVI, int DSID) {
    ...
    int pageSize = mi.getMaxRecords() <= 0 || mi.getMaxRecords() >= 10000 ? 10000 : mi.getMaxRecords()
    query.readAll(EXTCSR, 3, pageSize, {
      DBContainer recordEXTCSR ->
